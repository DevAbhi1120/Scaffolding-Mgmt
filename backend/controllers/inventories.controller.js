...
exports.createInventory = async (req, res) => {
  const { product_id, opening_stock, stock_in, stock_out, missing, damaged } =
    req.body;
  ...
  try {
    ...
    res.json({
      ...
      thumbnail_url: inventory.thumbnail_image
        ? `${req.protocol}://${req.get("host")}/uploads/inventories/${
...
exports.manualAdjust = (req, res) => {
  const { product_id, quantity, type, reason } = req.body;
  const data = { product_id, type: `MANUAL_${type.toUpperCase()}`, quantity: Math.abs(quantity), reason, user_id: req.user.id };
  const delta = type === 'in' ? +quantity : -quantity;

  InventoryTransaction.create(data, (err, result) => {
    if (err) return res.status(500).json({ error: err.sqlMessage });
    InventoryTransaction.updateBalance(product_id, delta, (err2) => {
      if (err2) return res.status(500).json({ error: err2.sqlMessage });
      res.json({ message: 'Manual adjustment complete', transactionId: result.insertId });
    });
  });
};

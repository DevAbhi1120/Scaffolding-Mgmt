...
nvm use 24.4.0
npm ci
Start MySQL & Redis (locally or via Docker). Quick Docker example:
Bash
...
Seed superadmin (if available, build then run compiled script or ts-node script):
Bash
...
mysql — MySQL 8
redis — Redis 7
In production you will prefer Dockerfile + k8s manifests (see k8s/).
...
Push to registry & deploy via k8s manifests or helm.
